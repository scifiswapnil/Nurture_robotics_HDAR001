...
class SwerveModule:
    ...
    def control_loop(self):
        ...
        # Priority: Turn-In-Place
        # If error > 20 degrees, kill translation to prioritize rotation
        # if abs(error_h) > 10.0:
        #     scale_factor = max(0.0, cos(radians(error_h)))
        ...
        # 4. Mixing
        # Use omega directly as diff_rpm to enable strong rotation
        # (Geometric scaling reduced gain by ~15x)
               
        left_rpm = V_out - omega
        right_rpm = V_out + omega
        print(f"Left: {left_rpm}, Right: {right_rpm}")
...
class SwerveDriveNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('track_width', 0.119) # Assuming square if not set, but explicit is better
                ...
                # PID Parameters
        self.declare_parameter('kp_heading', 0.60)
        self.declare_parameter('ki_heading', 0.020)
        self.declare_parameter('kd_heading', 0.040)
    ...
    def cmd_vel_callback(self, msg):
        ...
        wheels = [
            (vx - omega * (W / 2.0), vy + omega * (L / 2.0)), # FL (0) 'one'
            (vx + omega * (W / 2.0), vy + omega * (L / 2.0)), # FR (1) 'two'
            (vx - omega * (W / 2.0), vy - omega * (L / 2.0)), # RL (2) 'three'
            (vx + omega * (W / 2.0), vy - omega * (L / 2.0)), # RR (3) 'four'
        ]
